...
**Tests:** the first-run experience. With an empty workspace, the `pace` master runs a short setup wizard **before** any Discovery, writes `pace.config.toml`, then routes into Discovery in the chosen language. It configures; it never coaches.
...
- [ ] The `pace` master detects **zero-state** and runs the **onboarding wizard before Discovery** — it does not jump straight into the Discovery conversation.
...
- [ ] It then **routes into Discovery** (`pace-discovery`), now speaking the chosen language.
