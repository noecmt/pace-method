...
> **Case D is a two-step flow** (pure-orchestrator model). `pace-master` never turns prose into a signal id; the Analyst(`pace-debrief`) is the sole transformer of prose -> a structured signal in `log/`. So D unfolds as: master classifies the message as an *executed-training fact* -> **routes to the Analyst** -> the Analyst emits `sessions_skipped` -> master reads that signal and maps it via `signals.csv: sessions_skipped (3_weeks) -> partial_discovery_or_rolling` -> **proposes**. Since `pace-debrief` arrives in Sprint 4, the **Sprint-2 pass criterion for D is only the first step**: master routes to the Analyst without self-diagnosing. The downstream proposal is verified end-to-end at Sprint 4/5.
...
For each case, the chosen route matches the table. For D, the Sprint-2 check is the route to the Analyst (no self-diagnosis); the signal->proposal mapping against `signals.csv` is checked once `pace-debrief` exists (Sprint 4/5). Any mismatch => **fail** for that case.
