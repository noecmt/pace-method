...
- [ ] **Lean + intensity split.** The block carries exactly: `sessions {total, done, adjusted, skipped, pending}`, `adherence`, `duration_min {planned, actual}`, `intensity_split_min` (minutes by band **Z1-Z2 / Z3 / Z4-Z5**), a `read` (≤2 sentences, neutral), `generated_by`, `generated_at`, `status`.
- [ ] **Deterministic aggregation.** `duration_min.actual` sums `actual.duration_min_actual` over `done`/`adjusted` sessions; `intensity_split_min` assigns each executed session's actual minutes to the band of its **dominant (highest-intensity) planned zone**; `adherence = (done + adjusted) / (done + adjusted + skipped)` (pending excluded).
