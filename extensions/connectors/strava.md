...
A **read-only signal/data provider** via Strava's **official MCP** (remote, OAuth, subscribers-only). Consumed by `pace-checkin`, `pace-debrief`, `pace-rolling`. **Never** a persona, **never** called from `pace-master`, **never** generates a session, **never** written to (Strava is read-only).
...
- **Right table for the right signal** (Phase 2): same-day -> `adjustment-decisions.csv`; multi-week trend -> `signals.csv`; post-session execution -> `pace-debrief` (`learned_behavior` / log).
