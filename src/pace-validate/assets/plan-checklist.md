...
- [ ] **Declared-unavailable weekdays are empty (deterministic)**: for any availability constraint expressed as named weekdays in the vision or `profile.json` (hard constraint or `learned_behavior` — e.g. "no riding Saturday/Sunday", "Thursday unavailable"), **no planned session's `date` falls on one of those weekdays**, the weekday being computed from the date itself. A session on a declared-unavailable day => **not valid** (scenario 18). **`unplanned` sessions are exempt** — they record what the athlete actually executed, not a prescription (scenario 17).
