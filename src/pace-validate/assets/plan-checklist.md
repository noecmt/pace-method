...
- [ ] **Intensity legality**: every **planned** session in `weeks/*.json` uses only `allowed_intensity` for its phase and contains none of the `forbidden` items (deterministic vs CSV). E.g. no Z4/Z5 in base; no exhausting long ride in taper/race (scenarios 01, 04). **`unplanned` sessions (`planned: null`) are exempt** — they record what the athlete *executed* off-plan, not a prescription, so periodization legality does not apply (scenario 17).
...
- [ ] **Session dates inside their span (deterministic)**: every session in `weeks/<week_id>.json` has `start <= date <= end`, taken from that `week_id`'s row in `index.csv`. A session dated outside its own week => **not valid** (scenario 18).
- [ ] **`week_id` matches its span (deterministic)**: each row's `week_id` is the **ISO week label of its `start` date** (`BREAK` rows exempt). A label that disagrees with `start` => **not valid** — it is the usual symptom of a week silently re-aligned to Monday.
- [ ] **Declared-unavailable weekdays are empty (deterministic)**: when `profile.json` carries a `constraints[]` item of `type: "unavailable_weekdays"` (`value` = comma-separated lowercase English weekday names — contract: `extensions/_artefact_schema.md`), **no planned session's `date` falls on one of those weekdays**, the weekday being computed from the date itself. A session on a declared-unavailable day => **not valid** (scenario 18). **`unplanned` sessions are exempt** — they record what the athlete actually executed, not a prescription (scenario 17). An availability stated only in prose (vision text, free-form constraint `value`) is a **soft** finding instead — flag it for normalisation into the reserved shape, since prose cannot be gated.
...
- [ ] **Session discipline resolves (deterministic)**: every session's `sport` in `weeks/*.json` is a discipline declared in `profile.sports`. For a **planned** session, its `planned.target.range` is drawn from that discipline's zones block (`zones.by_discipline.<sport>`), with `target.metric` matching the sport pack's `primary_metric` family (`power`/`hr`/`pace`). An **`unplanned`** session (`planned: null`) has no `target` to resolve — only its `sport` must resolve to a declared discipline (scenario 17).
