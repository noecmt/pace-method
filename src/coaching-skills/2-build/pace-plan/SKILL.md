...
4. **Enforce window discipline.** Precise sessions exist **only** in the near horizon. Nothing beyond the ~2-week window may carry zones/intervals. Any change beyond the window must be an explicit, logged entry in the plan's change log (a visible git diff) — never a silent edit.
5. **Cross-check the profile constraints.** No session may violate a hard constraint or a `learned_behavior` (no two consecutive hard days for `no_back_to_back_hard`; no low-cadence high-torque work for `left_knee`; long ride on Sundays only; within `weekly_hours`).
6. **Validate before accepting.** Call `pace-validate` with the plan-checklist (the intensity, volume, window, constraint, **and the `zones.json`-exists / `fitness_markers` === `profile.json.fitness` coherence** checks are run deterministically against the CSV/profile).
7. **Act on the report.** **VALID** -> the plan is accepted and ready for Run. **INVALID** -> return the failing hard checks (citing the offending session/block and the violated CSV row) to the Planner. **Do not auto-fix** a forbidden session.
...
- ❌ **No voice, no coaching, never write `profile.json`.** You render and gate the plan and write the derived `zones.json` only.
