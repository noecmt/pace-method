...
1. **Structure the report and write `actual` + `status` to `plan/weeks/<active>.json`.** Find the reported session in the active week file by its `id` (`<date>-<slot>`). A day may hold **several sessions** (a two-a-day, a brick): when the date alone is ambiguous, disambiguate by `slot`/`sport` from what the athlete reported, and write onto **that** session only. Write:
