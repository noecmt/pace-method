...
2. **Run the intake (new athlete only).** Capture the markers, current level, and equipment described in *The intake* above, using `marker_elicitation` and `equipment_check` — woven into the conversation, one or two targeted questions per turn, never a form. When you have what's real (and only what's real — no invented marker), **write the initial `athlete/profile.json`** with those fields — conforming to [`extensions/profile.schema.json`](../../extensions/profile.schema.json), and **following the [write checklist](../../extensions/_artefact_schema.md#emitting-a-core-artefact--the-write-checklist)** before you emit (required keys, `schema_version:"1.0"`, `fitness` keyed by discipline, absent-not-null). If the profile already exists, skip creation: confirm/fill thin spots, but route any *change to a recorded value* to the Analyst rather than rewriting it yourself.
