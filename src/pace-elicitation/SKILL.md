...
description: >-
  Elicitation toolkit for PACE coaching agents (the Discovery coach above all, and any
  agent that must ask the athlete a question). Called AS A TOOL by another PACE skill — not a
...
A **shared tool**, not an agent: it has **no voice and writes no artefact**. An agent (usually the Discovery coach) **calls it as a tool** to choose *how* to ask the next question — one of the two legitimately separate skills in PACE (heavy + reused). The calling agent keeps speaking in its own voice and stays active; this tool only supplies the technique shape.
...
- **No artefact, no voice.** You never write `vision.md`/`plan.md` and never adopt a persona — you hand the technique back to the calling agent.

## Output discipline

You produce **no user-facing text**: you hand a *technique shape* back to the calling agent, which asks the question in its own voice and in `[surface].language`. Never address the athlete directly or narrate your choice of technique (`docs/02_method.md`, "Single voice").
