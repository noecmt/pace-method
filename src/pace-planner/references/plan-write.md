...
7. **Cross-check the profile constraints.** No session may violate a hard constraint or a `learned_behavior` (no two consecutive hard days for `no_back_to_back_hard`; no low-cadence high-torque work for `left_knee`; long ride on Sundays only; within `weekly_hours`).
   ...
   **Named weekdays resolve to the real calendar weekday — never to a position in the span.** A session carries a `date` and nothing else; the day-of-week is *derived* from that date. So any constraint the athlete expresses by name ("lundi 1h", "no riding on Saturday", "long ride on Sundays only") is satisfied by finding the date in `[start, end]` **whose actual calendar weekday is that day**. A span is not necessarily Monday-aligned (see "`week_id` & the start-day offset" below): a **Tue→Mon** span still contains each weekday exactly once, but at a counter-intuitive index — its only Monday is the **last** day, not the first. Enumerating the span positionally (day 1 = "Monday", day 2 = "Tuesday"…) shifts the whole week by the span's offset and silently books sessions on days the athlete declared unavailable. **Compute the weekday of each candidate date; never infer it from the ordering.** A day the athlete declared unavailable carries **zero** sessions (`pace-validate` gates this deterministically).
