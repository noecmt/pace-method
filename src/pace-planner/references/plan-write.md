...
7. **Cross-check the profile constraints.** No session may violate a hard constraint or a `learned_behavior` (no two consecutive hard days for `no_back_to_back_hard`; no low-cadence high-torque work for `left_knee`; long ride on Sundays only; within `weekly_hours`).
   ...
   **Named weekdays resolve to the real calendar weekday — never to a position in the span.** A session carries a `date` and nothing else; the day-of-week is *derived* from that date. So any constraint the athlete expresses by name ("lundi 1h", "no riding on Saturday", "long ride on Sundays only") is satisfied by finding the date in `[start, end]` **whose actual calendar weekday is that day**. A span is not necessarily Monday-aligned (see "`week_id` & the start-day offset" below): a **Tue→Mon** span still contains each weekday exactly once, but at a counter-intuitive index — its only Monday is the **last** day, not the first. Enumerating the span positionally (day 1 = "Monday", day 2 = "Tuesday"…) shifts the whole week by the span's offset and silently books sessions on days the athlete declared unavailable. **Compute the weekday of each candidate date; never infer it from the ordering.** A day the athlete declared unavailable carries **zero** sessions. When the athlete states a *recurring* weekly unavailability, make sure it is recorded in `profile.json` as the reserved constraint `type: "unavailable_weekdays"` (`value` = comma-separated lowercase English weekday names — contract: [`_artefact_schema.md`](../../../extensions/_artefact_schema.md#weekly-availability--the-unavailable_weekdays-constraint)); that shape is what lets `pace-validate` and the host gate it deterministically. Prose in the vision is advisory and cannot be gated.
...
**`week_id` & the start-day offset.** Use the **ISO week label** of the week's `start` date as `week_id` (e.g. `2026-W24`); keep the athlete's **real span** in `start`/`end` (a plan that starts on a Tuesday keeps Tue->Mon spans). The active week is whichever row satisfies **today ∈ [`start`,`end`]**, so **week *selection*** needs no recalibration when the span is not Monday-aligned. This migration runs **once**; thereafter the window advances via the `rolling` capability (accumulate, never overwrite).

> ⚠️ **This tolerance covers week selection only — never session placement.** A non-Monday span does **not** license a day-shifted week: each session's weekday is still derived from its own `date`, and a named weekday still resolves to the date whose real calendar weekday matches (step 7 above). If an athlete reports that the days are off by one, the fault is in the **placement**, not in the span — re-resolve every named day against the calendar and amend the affected week. Do not cite this paragraph as a reason to leave a day-shifted week uncorrected.
