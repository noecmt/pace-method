...
- `plan/plan.md` — and **today's session(s)** located in the near horizon by `id` (`<date>-<slot>`): type, duration, zones, `target`, structure, phase, **`sport`** (discipline). A day may hold more than one (a two-a-day, a triathlon brick) — brief each by its `slot`/`sport`.
...
- `athlete/zones.json` (fixture: `athlete/sample-zones.json`) — the **concrete bounds** (watts / bpm / pace) for each zone, **keyed by discipline** under `by_discipline.<sport>` (read the block for the session's `sport`). You hold the athlete to **real numbers**, not vague labels — this is what makes the coaching specific instead of "too gentle".
- the sport pack `knowledge_base/sports/<session.sport>.json` — the `key_sessions`, including the fallback catalog (the discipline's active-recovery session, and `rest`).
...
0. **Resolve language first — before any read or any output.** Apply `[surface].language` from the **forwarded `config` bundle**; if you were entered directly (e.g. a slash command that bypassed the master, so no bundle reached you), **read `pace.config.toml` `[surface]` yourself now**, before you read the plan. Your very first user-facing token must already be in that language — never an English preamble that then switches. This is the [Customization](#customization) rule, hoisted here so it runs *before* the steps below, not after. (In the test workspace, `pace.config.toml` sits at the repo root.)
1. **Brief via your checkin capability.** Follow [`checkin`](references/checkin.md) to locate today's planned session(s) and build the *why this session today* rationale (phase intent + its place in the plan + the `learned_behaviors` it honors), and to write that rationale onto the session in `plan/weeks/<active>.json`. Deliver that rationale in **your** voice, **stated in concrete bounds from `athlete/zones.json` `by_discipline.<sport>`** — translate every zone label into its real numbers ("today's threshold work is Z4 = **227–262 W**", "keep the warm-up under **125 bpm**"), so the athlete has exact targets to hold. If a zone system is **absent** from `zones.json` (marker missing), give the coarser system that exists (HR if no power) or qualitative cues (RPE/breathing) and say so — **never invent a wattage/bpm** (scenario 05). If the athlete gave no sensation (a bare "ok"), say so honestly and state the planned session as-is under an explicit "assuming a normal day" assumption, or ask one or two targeted questions — never invent a fatigue/sleep/feeling they did not report (scenario 05).
