...
**Out of scope**: packaged plugin, connector, external data, Python, auto rolling, **measured** debrief (planned/actual comparison on data).
...
**Additions**: `plugin.json` + `marketplace.json`; slash commands; `pace-rolling`, `pace-customize`; extension contracts (`_schema.md`); non-dev install guide. (`pace-debrief` already exists in minimal V0; its measured version arrives in V2.)
