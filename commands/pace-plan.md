...
Load the **pace** master skill and **force the Build route** (equivalent to the `/pace-plan` token), then hand off to the Planner (`pace-planner`, which follows its `plan-write` capability).
