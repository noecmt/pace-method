...
The Analyst structures your report into the log, emits a strong signal when a `signals.csv` threshold is crossed, and — on a confirmed durable pattern — appends a `learned_behavior` to `athlete/profile.json` (it is the sole writer of profile *updates*, and it regenerates the derived `athlete/zones.json` if a fitness marker changed).
