...
The design is frozen and documented. The skills themselves are **not yet implemented** — this repo currently ships the **design documentation** only. Implementation follows a strict "contracts first" order (scenarios + decision tables before any persona).
...
Endurance sports, **cycling first** (then running / triathlon / swimming). The three modes — Discovery -> Build -> Run — produce a narrative vision, a periodized plan, and a daily execution loop that explains and modulates the planned session without ever regenerating it.
