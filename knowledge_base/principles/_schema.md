...
- **Wired, not orphaned.** A principle that should inform reasoning is referenced from the relevant persona's *Inputs* as load-on-demand (e.g. the Planner reads `periodization`/`polarized_training`/`progressive_overload`; the Run coach/`pace-adjust` read `recovery_basics`). An unreferenced principle is dormant knowledge, not a behaviour change.
