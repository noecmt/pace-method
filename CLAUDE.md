...
- `log/signals.md` — the **cross-session signals ledger**: append-only bullets the Analyst emits when a `signals.csv` threshold fires, and the only thing the `pace` master reads to propose re-Discovery/rolling. Per-session debriefs are **not** here — they live on the session object in `plan/weeks/<week>.json` (the single home for a session's whole lifecycle: `planned`, `rationale`, `actual`, `debrief`, `adjustment` — filled in place; past weeks are immutable history).
...
session verbatim) or **hands off to exactly one voiced agent** (the route lane: auto when
