...
- `athlete/profile.json` — structured long-term state, including `learned_behaviors`. **Created once by the Discovery intake** (markers, current level, equipment — seeded only from what the athlete actually gave). Thereafter the Analyst (`pace-analyst`) is the **sole writer of updates** (and of `learned_behaviors`). Two writers, two moments — intake creates, the Analyst updates; no other agent writes it. On a plannable fact, `profile.json` is authoritative for the Planner; the Vision carries the *why*. The athlete declares `sports[]` (a list — one discipline, or three for a triathlete) and `fitness` is **keyed by discipline** (`fitness.<sport>`); the programme `current_phase` is sport-agnostic and sits at the top.
- `athlete/zones.json` — the **derived** artefact: intensity zones in concrete bounds (watts/bpm/pace), **keyed by discipline** under `by_discipline`, generated from `profile.json.fitness.<discipline>` + the sport pack. **Never hand-edited**; written by the planner's `plan-write` capability, regenerated whole by the Analyst on a marker change.
- `log/signals.md` — the **cross-session signals ledger**: append-only bullets the Analyst emits when a `signals.csv` threshold fires, and the only thing the `pace` master reads to propose re-Discovery/rolling on a *signal*. Per-session debriefs are **not** here — they live on the session object in `plan/weeks/<week>.json` (the single home for a session's whole lifecycle: `planned`, `rationale`, `actual`, `debrief`, `adjustment` — filled in place; past weeks are immutable history). Each session is keyed by `id` (`<date>-<slot>`), not by `date` alone — **a day may hold several sessions** (a two-a-day, a triathlon brick), each carrying its own `sport` (discipline) and a metric-agnostic `planned.target` (`{metric, zone_ref, range}`). That same week file also carries a **week-level `summary`** (sibling of `sessions`): a derived aggregate (counts, adherence, durations, `distance_km` **keyed by sport**, `intensity_split_min`, a `by_sport` block only when the week spans >1 sport, a neutral `read`) the Analyst refreshes idempotently on each debrief and the master recites in the concierge lane — derived, never fabricated; the Planner never writes it. The week/profile/zones files carry a `schema_version`; their **frozen contract** is `extensions/_artefact_schema.md`. The master's *other* proposal source is **plan-state, not a signal**: when `plan/index.csv` shows no near week planned after the active one, it proactively proposes rolling (`/pace-plan`) — never via `log/signals.md`.
